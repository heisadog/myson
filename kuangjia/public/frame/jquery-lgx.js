	...
	$.fn.footer = function (options) {
	    ...
	    return this.each(function () {
	        ...
	        for(var i = 0; i<opt.butName.length; i++){
	            forhtml+='<div data-url="'+opt.butUrl[i]+'"><span>'+opt.butIcon[i]+'</span>'+opt.butName[i]+'</div>';
	        }
	        var html = '<footer>'+forhtml+'</footer>';
	        $(this).append(html);
	        $(this).find("footer").css('background-color',opt.butCss.backgroundColor).css('color',opt.butCss.color);
	        $(this).find("footer div").eq(opt.butCheck).addClass("main_color");
	        $(this).find("footer div").css('width',100/opt.butName.length+"%");
	    })
	}
})(jQuery)
+(function ($) {
